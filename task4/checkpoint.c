...
static
void
calculate (double** matrix, int iterations, int threads, int sync)
{
	int i, j, k, l;
	int tid;
	int lines, from, to;
			...
			{ // write
				...
				for (i = from; i < to; i++)
				{
					...
					while (nb < (N * sizeof(double)))
					{
						...
						ret = pwrite(fd, (char*)matrix[i] + nb, (N * sizeof(double)) - nb, (i * N * sizeof(double)) + nb);
...
{
	int threads, iterations, sync;
	double** matrix;

	if (argc < 4)
	{
		printf("Usage: %s threads iterations sync\n", argv[0]);
		printf("       sync=0: no fsync\n");
		printf("       sync=1: fsync after every pwrite\n");
		printf("       sync=2: fsync after every iteration\n");
		exit(EXIT_FAILURE);
	}
	else
	{
	...
	calculate(matrix, iterations, threads, sync);
